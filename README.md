...
用 **Rust + 纯 Win32 原生 API** 实现，**直接解码上游全套 91 段 640×360 透明 webm**（VP9 + alpha，与 Web 插件素材同步更新）。
...
| **本版** | **Rust + libvpx 静态链接 + Win32 API** | **~48MB 单文件** | **零**（素材已内嵌） | 秒开 |

程序本体仅约 1MB，其余约 46MB 为内嵌的 91 段高清 webm 动画（用户可接受素材体积）。
...
```sh
# 素材（assets/videos/，91 个 webm，与 Web 插件 dsh-pet assets/thumb 同步）已包含在本仓库，无需手动获取。
...
│   └── monitor.rs      # 屏幕工作区
├── build.rs            # 素材打包（91 webm → assets.pak + 索引）
├── tools/              # 验证/诊断脚本
...
- `cargo run --bin decode_check`：解析 91 个动画（21931 帧）、alpha 分布校验、解码性能（<40ms/帧）
